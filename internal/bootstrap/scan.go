...
// one PDMC file. Thin wrapper around ScanOne that merges results — see
// ScanOne for the exclusion layering and per-root walk semantics.
func Scan(store *classifier.ExcludeStore, matcher *excludes.Matcher, roots []string) ([]string, error) {
	seen := make(map[string]bool)
	for _, root := range roots {
		found, err := ScanOne(store, matcher, root)
		if err != nil {
			return nil, err
		}
		for _, d := range found {
...
// ScanOne walks a single root and returns directories inside it that contain
// at least one PDMC file. Excluded directories are skipped. Three exclusion
// layers compose here, in order:
//
...
//
// Used by Scan at daemon startup and by runtime discovery (root-watch
// consumer + periodic rescan) so both paths go through identical
// classification. Missing/unreadable root is not an error — WalkDir's
// initial error is mapped to fs.SkipDir, yielding an empty slice.
func ScanOne(store *classifier.ExcludeStore, matcher *excludes.Matcher, root string) ([]string, error) {
	seen := make(map[string]bool)
